...
from game.lib.components import Bird
from game.lib.level import Level
from game.lib.physics import Physics
...
class Game:
    ...
    def sling_action(self):
        ...
        v = Physics.vector((self.sling_x, self.sling_y), (self.x_mouse, self.y_mouse))
        uv = Physics.unit_vector(v)
        uv1 = uv[0]
        uv2 = uv[1]
        self.mouse_distance = Physics.distance(self.sling_x, self.sling_y, self.x_mouse, self.y_mouse)
    ...
    def draw_level_cleared(self):
        level_cleared = self.bold_font3.render("Level Cleared!", 1, self.WHITE)
        score_level_cleared = self.bold_font2.render(str(self.score), 1, self.WHITE)
    ...
    def draw_level_failed(self):
        failed = self.bold_font3.render("Level Failed", 1, self.WHITE)
